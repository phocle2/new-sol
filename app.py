...
if not SECRET_B58:
    raise RuntimeError("Missing REWARD_WALLET_SECRET_BASE58 env var")
...
if len(secret_bytes) != 64:
    raise RuntimeError(f"REWARD_WALLET_SECRET_BASE58 must decode to 64 bytes, got {len(secret_bytes)}")
...
def reward_send():
    ...
    if idem_key:
        ...
        if k in PAID:
            sig, _ = PAID[k]
            return jsonify({"ok": True, "signature": sig, "already_paid": True})

    lamports = int(amount_sol * LAMPORTS_PER_SOL)

    try:
        blockhash = client.get_latest_blockhash().value.blockhash
        ix = transfer(TransferParams(from_pubkey=payer_pubkey, to_pubkey=to_pubkey, lamports=lamports))
